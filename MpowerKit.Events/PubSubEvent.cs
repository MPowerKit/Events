...
public class PubSubEvent : EventBase
{
    ...
    public SubscriptionToken Subscribe(Action action, bool keepSubscriberReferenceAlive)
    {
        ...
        return InternalSubscribe(new EventSubscription(actionReference));
    }
    ...
    public virtual void Unsubscribe(Action subscriber)
    {
        lock (Subscriptions)
        {
            ...
            if (eventSubscription is null) return;
    ...
    public virtual bool Contains(Action subscriber)
    {
        lock (Subscriptions)
        {
            return Subscriptions.Cast<EventSubscription>().FirstOrDefault(evt => evt.Action == subscriber) is not null;
        }
    }
}
...
public class PubSubEvent<TPayload> : EventBase
{
    ...
    {
        return Subscribe(action, false, filter);
    }
    ...
    {
        var actionReference = new DelegateReference(action, keepSubscriberReferenceAlive);
        var filterReference = filter is not null
        ...
        return InternalSubscribe(new EventSubscription<TPayload>(actionReference, filterReference));
    }
    ...
    public virtual bool Contains(Action<TPayload> subscriber)
    {
        lock (Subscriptions)
        {
            return Subscriptions.Cast<EventSubscription<TPayload>>().FirstOrDefault(evt => evt.Action == subscriber) is not null;
        }
    }
}